...
import gameCharacters.AssignSpecialAttacks;
...
import gameCharacters.MonsterFactory;
//import item.*;
import specialAttack.SpecialBuff;
...
public class DungeonDriver {
	...
	private static void combat(final DungeonCharacter monster) {
	...
	private static void userPotion(final Hero hero, final Dungeon dungeon){
		if(hero.getVisionPotionCount() > 0 || hero.getHealPotionCount() > 0 || hero.getManaPotionCount() > 0){
			...
			if(response.equals("Y")){
				...
				if(potionResponse.equals("VP")){
					...
					hero.decreaseVisionPotionCount();
	...
	private static void healTurn(final Hero hero, final DungeonCharacter monster) {
	...
	private static void damageExchange(final ArrayList<DungeonCharacter> fightOrder) {
	...
	private static void printHealthState(final DungeonCharacter hero, final DungeonCharacter monster) {
